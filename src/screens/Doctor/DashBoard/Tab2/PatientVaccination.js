...
export default class PatientVaccination extends Component {
  constructor(props) {
    super(props);
    this.state = {
        vacc: {},
...
render() {
    ...
    return (
                ...
                renderItem={({item, index, separators})=> {
                    return (
                        <View style={{marginLeft:10, marginBottom:20,padding:5}}>
                            ...
                            onPress={()=>this.props.navigation.navigate('VerifyVaccination', {"vaccItem" : item, "vacc": this.state.vacc})}
                            >
                                <View style = {{flex:1}}>
                                    <View style = {{width:25, height: 25, margin:5 ,backgroundColor: item.isverified?"green":'#C4C4C4', borderRadius:100}}>
        ...
        <TouchableOpacity style = {{position: 'absolute', bottom: 10, right: 10}}
            onPress={()=>this.props.navigation.navigate('AddVaccination', {"vacc": this.state.vacc})}
            >
            <Icon
                raised
                name='plus'
